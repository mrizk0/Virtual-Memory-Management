...
// ENABLE THIS CODE TO START TO TEST YOUR PAGING IMPLEMENTATION
    ...
    nk_vc_printf("Attempting to add eager region to address space\n");
    if (nk_aspace_add_region(mas, &r)) {
        nk_vc_printf("failed to add initial eager region to address space\n");
        goto vc_setup;
    }
    nk_vc_printf("Survived adding eager region to address space\n");
    ...
    // Poke around at a few places in the virtual address space to see if they
    // match. This will fault in pages as we go. We look at two places to make
    // sure the paging implementation works.
    // 0xffff80000.....+ 1 MB:
    nk_vc_printf("Starting memory comparisons\n");
    ...
    nk_vc_printf("Done with first memcmp\n");
    // 0xffff80000.....+ 1 GB:
